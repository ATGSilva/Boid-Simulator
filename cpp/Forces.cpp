...
void WallForce(Boid& boid)
{
    ...
    Vec3D pos_bound_vec = Vec3D(x_ubound, y_ubound, z_ubound);
    Vec3D neg_bound_vec = Vec3D(x_lbound, y_lbound, z_lbound);
    Vec3D wall_force = Vec3D();

    if (x_ubound || y_ubound || z_ubound)
    {   
        wall_force = wall_force + (-1) * (WALL_FORCE * pos_bound_vec) * (boid.pos * boid.pos);
    }
    if (x_lbound || y_lbound || z_lbound)
    {
        wall_force = wall_force + WALL_FORCE * neg_bound_vec * (boid.pos * boid.pos);
    }
